...
- `deploy/chart/` contains the Kubernetes Helm chart. Production expects a DB-provisioner-created secret via `database.existingSecret`; chart-managed Postgres is for temporary review only. The live review app workload belongs on `cr-k3s`; DB provisioning and the public edge gateway are managed on `crc-k3s`.
...
- Do not commit real secrets. `DATABASE_URL` must come from the DB provisioner or an external secret in production; `SESSION_SECRET` must come from deployment secrets. The app chart consumer is managed in `/home/andrew/entities/cr/technology/infra-as-code`; the CRC DB provisioner and public gateway are managed in `/home/andrew/entities/cr/technology/cloud-as-code`.
